...
class FactCheckService:
    ...
    async def check_claim(self, claim_text: str, language: str = "en") -> Dict[str, Any]:
        """
    ...
    def _process_response(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for claim in claims[:5]:  # Process top 5 matching claims
            ...
            rating: str = str(claim_review.get("textualRating", ""))
            publisher_data = claim_review.get("publisher", {})
            publisher: str = publisher_data.get("name", "Unknown") if isinstance(publisher_data, dict) else "Unknown"
            url: str = str(claim_review.get("url", ""))
            
            # Map textual ratings to numeric score
            score_value = self._rating_to_score(rating)
            score: Optional[int] = score_value
            
            if score_value is not None:
                # Use explicit addition to help type checker
                val: int = score_value
        ...
        veracity_score: Optional[int] = None
        if scored_count > 0:
            veracity_score = round(total_score / scored_count)
